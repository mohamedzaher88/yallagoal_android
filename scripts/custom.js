...
$(document).ready(function () {      
    ...
    function init_template(){//Class is vital to run AJAX Pages   
		...
		if ( page_title == 'index' ){ //index page
			onloading();
		} 
        if ( page_title == 'end' ){ //Soon page
			$('.back-to-top-badge').hide()
		} 
        if ( page_title == 'soon' ){ //Soon page
			
			var newdate = localStorage.getItem("matchtime")
			setdate(newdate)
			$('.back-to-top-badge').hide()
		} 
		if ( page_title == 'home' ){ //home page
		$('.thumb-layout1').hide()
			 setTimeout(function(){  
